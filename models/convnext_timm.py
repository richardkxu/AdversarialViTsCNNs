...
default_cfgs = dict(
    ...
    convnext_tiny_adv=_cfg(url="https://dl.fbaipublicfiles.com/convnext/convnext_tiny_1k_224_ema.pth"),
...
class AdvConvNeXt(ConvNeXt):
    def __init__(
        ...
        super().__init__(in_chans=in_chans, num_classes=num_classes, global_pool=global_pool,
                         output_stride=output_stride, patch_size=patch_size,
                         depths=depths, dims=dims, ls_init_value=ls_init_value, conv_mlp=conv_mlp,
                         head_init_scale=head_init_scale, head_norm_first=head_norm_first, norm_layer=norm_layer,
...
def _create_adv_convnext(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        AdvConvNeXt, variant, pretrained,
        default_cfg=default_cfgs[variant],
        pretrained_filter_fn=checkpoint_filter_fn,
        feature_cfg=dict(out_indices=(0, 1, 2, 3), flatten_sequential=True),
        **kwargs)
...
@register_model
def convnext_tiny_adv(pretrained=False, **kwargs):
    model_args = dict(depths=(3, 3, 9, 3), dims=(96, 192, 384, 768), **kwargs)
    model = _create_adv_convnext('convnext_tiny', pretrained=pretrained, **model_args)
    return model
